...
const useSpecificExam = ({ Title }) => {
  ...
  const fetchSpecificExam = () => {
    getSpecificExam({ Title }).then((data) => {
      const specificExamWithoutId = data.$values.map(({ $id, questions, certificate, ...rest }) => rest);      // Exclude $id, questions, certificate from data
...
const useQuestionsForExam = ({ Title }) => {
  ...
  const fetchQuestionsForExam = () => {
    getQuestionsForExam({ Title }).then((data) => {
      const QuestionsForExamWithoutId = data.$values.map(({ $id, exam, ...rest }) => rest);      // Exclude $id, exam from data
...
const useCandidateExam = ({ Title, CandidateNumber }) => {
  ...
  const fetchCandidateExam = () => {
    getCandidateExam({ Title, CandidateNumber }).then((data) => {
      const candidateExamWithoutId = data.$values.map(({ $id, ...rest }) => rest);      // Exclude $id, exam from data
