...
const columns = [
    ...
    { id: 'certificateId', label: 'Certificate ID', minwidth: 170, width: "auto" },
    ...
    { id: 'updateButton', label: 'UPDATE', minwidth: 10, width: 170, },
    { id: 'deleteButton', label: 'DELETE', minwidth: 10, width: 170, },
];

function createData(id, examId, certificateId, title, description, time) {
    const updateButton = "UPDATE"
    const deleteButton = "DELETE"
    return { id, examId, certificateId, title, description, time, updateButton, deleteButton };
}
...
export default function StickyHeadTable() {
    ...
    return (
        <Paper sx={{ width: '80%', overflow: 'hidden', position: "relative", margin: "30px auto" }}>
                                            ...
                                            return (
                                                <TableCell key={column.id} align="center">
                                                    {(value === "UPDATE" || value === "DELETE") 
                                                        ? <Button variant="contained" color={value==="UPDATE"? "success":"error"} onClick={value==="UPDATE"? handleUpdate:handleDelete}>{value}</Button>
                                                        : <>{value}</>}
                                                </TableCell>
                                            );
                                        })}
